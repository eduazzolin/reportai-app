...
public class StartSeeder implements CommandLineRunner {
   ...
   private void loadCategoria() {
      if (categoriaRepository.count() == 0) {
         categoriaRepository.save(Categoria.builder().nome("Buraco na rua").icone("/markers/street.svg").build());
         categoriaRepository.save(Categoria.builder().nome("Lâmpada queimada").icone("/markers/traffic-light.svg").build());
         categoriaRepository.save(Categoria.builder().nome("Calçada danificada").icone("/markers/path.svg").build());
         categoriaRepository.save(Categoria.builder().nome("Poda de árvore").icone("/markers/tree.svg").build());
         categoriaRepository.save(Categoria.builder().nome("Vazamento de água").icone("/markers/flood.svg").build());
         categoriaRepository.save(Categoria.builder().nome("Sinalização de trânsito").icone("/markers/traffic-sign.svg").build());
         categoriaRepository.save(Categoria.builder().nome("Poluição sonora").icone("/markers/traffic-sign.svg").build());
         categoriaRepository.save(Categoria.builder().nome("Lixo acumulado").icone("/markers/traffic-sign.svg").build());
         categoriaRepository.save(Categoria.builder().nome("Outros").icone("/markers/general.svg").build());
         ;
      }
   }
   ...
   private void loadUsuario() {
      if (usuarioRepository.count() == 0) {
         usuarioRepository.save(Usuario.builder().role("ADMIN").nome("User").email("Admin").senha("123456").cpf("111111").build());
         usuarioRepository.save(Usuario.builder().role("USER").nome("João Silva").email("joao@silva.com").senha("123456").cpf("222222").build());
         usuarioRepository.save(Usuario.builder().role("USER").nome("Maria Marques").email("maria@marques.com").senha("123456").cpf("333333").build());
         usuarioRepository.save(Usuario.builder().role("USER").nome("Joaquim Silva").email("joaquim@silva.com").senha("123456").cpf("444444").build());
         usuarioRepository.save(Usuario.builder().role("USER").nome("Márcio Mendes").email("marcio@mendes.com").senha("123456").cpf("555555").build());
      }
   }
   ...
   private void loadRegistro() {
      if (registroRepository.count() == 0) {
         registroRepository.save(Registro.builder()
                 ...
                 .localizacao("Rua 1")
                 .latitude(new BigDecimal("-27.5884"))
                 .longitude(new BigDecimal("-48.5229"))
